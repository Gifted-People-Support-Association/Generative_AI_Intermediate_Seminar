...
**Dify（ディファイ）** は、生成AIアプリケーションを素早く開発・デプロイできるオープンソースのプラットフォームです。

**主な特徴:**
* **ノーコード/ローコード開発** - ビジュアルなワークフローエディタで直感的にAIアプリを構築
* **マルチモデル対応** - OpenAI、Anthropic、Google Gemini、ローカルLLMなど様々なモデルをサポート
* **RAG機能内蔵** - ドキュメントをアップロードするだけで知識ベース（Knowledge）を構築可能
* **エージェント機能** - 外部APIやツールと連携して自律的なAIアシスタントを構築
* **簡単デプロイ** - WebチャットボットやAPIとして即座に公開可能
...
Difyで作成できるアプリケーションタイプ：

1. **Chatbot（チャットボット）**
   * 複数ターンの会話が可能な対話型アシスタント
   * 会話履歴を保持し、文脈を理解した応答が可能
   * カスタマーサポートやFAQボットに最適

2. **Text Generator（テキストジェネレーター）**
   * 単発のテキスト生成タスク向け
   * 翻訳、要約、コンテンツ生成などに使用
   * ワンショットで結果を返す

3. **Agent（エージェント）**
   * ツール呼び出し機能を持つ自律的なアシスタント
   * 外部APIやデータベースにアクセス可能
   * 複雑なタスクを自動化
...
3. 以下の情報を入力：
   ...
   * **Model Name:** デフォルトのままでOK（例: `gemini-pro`）
...
* 利用可能なモデルの一覧が表示されます：
  ...
  * `gemini-2.0-flash` - 高速・低コストモデル
  * その他

> **💡 ヒント:** 初めての場合は `gemini-2.0-flash` を推奨します（高速で無料枠が広い）
...
1. **異なるモデルの比較**
   * `gemini-2.0-flash` と `gemini-1.5-pro` で同じ質問をして、応答の違いを比較
