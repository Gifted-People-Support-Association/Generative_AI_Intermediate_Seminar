...
### PDF / Excel 変換後に文字が正しく表示されない場合

ここでは、skill-creator によって作成した独自スキルが、PDF または Excel 形式への変換後に発生する表示トラブルへどのように対応できるかを考える。

議事録データを PDF や Excel に変換した場合、変換処理自体は成功していても、生成されたファイルを開くと日本語が正しく表示されないことがある。たとえば、文字化け、豆腐文字、空白化、文字欠落、意図しない記号への置換、Excel セル内の文字の見切れなどが発生する場合がある。

このような問題は、議事録の内容そのものではなく、ファイル変換時のフォント、文字エンコーディング、セル幅、行高、折り返し設定、PDF のフォント埋め込みなどに起因することが多い。

そのため、独自スキルを作成する際には、単にファイルを生成するだけでなく、変換後のファイルを確認し、文字表示に問題がある場合の再生成や報告までをワークフローに含めることが重要である。

### 発展演習：既存の議事録作成・ファイル変換スキルを改善する

ここでは、すでに作成した議事録作成・ファイル変換スキルを対象に、運用中に見つかった問題をもとにスキルを改善する演習を行う。

スキルは一度作成して終わりではなく、実際に使用した結果を確認し、不足している処理や確認観点を追加することで、より実務で使いやすいものに改善できる。

たとえば、議事録データを PDF や Excel 形式に変換したあと、次のような問題が発生する場合がある。

- PDF に変換した後、日本語が文字化けしている
- PDF 上で一部の文字が空白や豆腐文字として表示される
- Excel に変換した後、セル内の文字が見切れている
- Excel の列幅や行高が不足し、議事録の内容を読みづらい
- 表内の日本語、記号、改行が意図したとおりに表示されていない

このような問題が発生した場合は、既存スキルの `SKILL.md` に、変換後ファイルの品質確認、文字表示の検証、必要に応じた再生成の手順を追加する。

#### 改善用プロンプト例
```markdown
以下は、すでに作成済みの議事録作成・ファイル変換スキルです。

この既存スキルを改善してください。

## 改善したい背景

現在のスキルでは、会議の発言ログや文字起こしデータをもとに議事録を作成し、指定された形式に応じて Excel 形式、Word 形式、PDF 形式へ変換できます。

しかし、PDF または Excel 形式へ変換した後に、文字が正しく表示されない場合があります。

たとえば、次のような問題が発生することがあります。

- PDF 内の日本語が文字化けする
- PDF 内の一部の文字が空白、豆腐文字、意図しない記号として表示される
- Excel のセル内で文字が見切れる
- Excel の列幅、行高、折り返し設定が不十分で内容を読みづらい
- 表内の日本語、記号、改行が正しく表示されない
- 元の議事録データには存在する文字が、変換後ファイルでは欠落している

## 改善要件

既存の `SKILL.md` を読み取り、現在の処理内容を維持したまま、次の改善を加えてください。

- PDF または Excel 形式への変換後に、文字表示が正しく行われているか確認するステップを追加してください。
- PDF 出力時は、日本語表示に対応したフォントを使用するようにしてください。
- PDF 出力時に可能であれば、フォント埋め込みを行うようにしてください。
- PDF で文字化け、豆腐文字、空白化、文字欠落が見つかった場合は、日本語対応フォントを指定し直して再生成する手順を追加してください。
- Excel 出力時は、日本語表示に対応したフォントを使用してください。
- Excel 出力時は、列幅、行高、折り返し表示、セル内改行が適切に設定されているか確認してください。
- Excel で文字が見切れている場合は、列幅の自動調整、折り返し表示、行高調整を行って再生成する手順を追加してください。
- 変換後ファイルの文字表示に問題がある場合は、問題が発生したファイル形式、対象ページまたはシート名、該当箇所、推定原因、実施した修正内容をユーザーへ報告するようにしてください。
- 自動修正できない場合は、手動確認が必要な箇所を明示するようにしてください。

## 追加してほしい処理の流れ

既存の処理フローに、次のステップを追加してください。

1. 議事録本文を生成する
2. 指定されたファイル形式へ変換する
3. 変換後ファイルの文字表示を確認する
4. 文字化け、文字欠落、セル内文字の見切れ、表の崩れがないか確認する
5. 問題がある場合は、フォント、列幅、行高、折り返し表示、改ページなどを調整して再生成する
6. 再生成後も問題が残る場合は、問題箇所と手動確認事項をユーザーに提示する
...
- PDF 変換時のテストケース
- Excel 変換時のテストケース
- 改善後のスキルが正しく動作したと判断する成功条件

既存スキルの目的や基本的な処理の流れは大きく変更せず、PDF または Excel 変換後の文字表示確認と再生成手順を追加する形で改善してください。
