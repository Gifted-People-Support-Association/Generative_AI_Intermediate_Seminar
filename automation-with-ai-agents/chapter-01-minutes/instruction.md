...
### PDF形式への変換までを自動化するケースの手順

Anthropic の公式 pdf スキルは、PDF の読取・編集・結合・分割・フォーム入力に加え、新規 PDF 作成にも対応している。したがって、AI が生成した議事録データをもとに PDF を直接生成する運用は可能である。

ただし、複雑なレイアウトやテンプレート準拠の文書を安定して出力するには、見出し構成、表組み、改ページ、余白、フォント等の条件を詳細に指定し、生成結果を十分に検証する必要があり、PDFは修正性が低いため、実務利用では Word などの編集可能形式で内容確定後に PDF化する運用の方が適する場合もある。
...
指示プロンプト
```txt
会議の発言ログや文字起こしデータを入力として受け取り、指定されたフォーマットで議事録を作成し、その後に指定されたファイル形式へ変換できる新しいスキルを作成してください。
